...
void loop() {
  ...
  if (currentMillis - lastMillis < 50L) return;
...
ISR (ANALOG_COMP_vect) {
  ...
  for (int i=0; i<10; i++) {            // We check the comparator 10 times just to be sure
    if (ACSR & 0x01) {                  // If step is with IRQ on rising BEMF, ACO should be 1
      if (0 == (ACSR & (1<<ACO))) {
        i--;  // ACO = 0, check again
        Serial.print("#"); Serial.println(ACSR, BIN);
      }
    } else {                            // If step is with IRQ on falling BEMF, ACO should be 0
      if (0 != (ACSR & (1<<ACO))) {
        i--;  // ACO = 1, check again
        Serial.print("+"); Serial.println(ACSR, BIN);
      }
